...
Measured starting model for ROS 2 Humble, RViz and Gazebo Classic.
...
## Install in the existing workspace

Extract/copy the `hege_description` directory into:

```text
/home/agtubot/hege_ws/src/hege_description
```
...
Install missing dependencies once if required:
...
```bash
find ~/hege_ws/src -name package.xml -print | xargs grep -l '<name>hege_description</name>'
```
